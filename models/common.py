...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        # class names
        if 'names' not in locals():  # locals()以字典形式返回当前位置全部局部变量
            names = yaml_load(data)['names'] if data else {i: f'class{i}' for i in range(999)} # 获取类名 data为data/coco128.yaml
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch 此时的model是DetectionModel类，所以会调用这个类的forward(img,augment,visualize)
